...
export async function up({ db, payload }: MigrateUpArgs): Promise<void> {
  ...
  for (const stmt of statements) {
    try {
      await payload.db.drizzle.execute(sql`${sql.raw(stmt)}`)
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'unknown error'
      console.warn(`[migration] Skipped statement (${msg}): ${stmt.slice(0, 80)}...`)
    }
  }
  ...
  for (const stmt of versionTables) {
    try {
      ...
      const msg = err instanceof Error ? err.message : 'unknown error'
      console.warn(`[migration] Skipped version table (${msg}): ${stmt.slice(0, 80)}...`)
    }
  }
  ...
  for (const stmt of parentTables) {
    try {
      ...
      const msg = err instanceof Error ? err.message : 'unknown error'
      console.warn(`[migration] Skipped parent table (${msg}): ${stmt.slice(0, 80)}...`)
    }
  }

  // --- Version breadcrumbs table ---
  try {
    await payload.db.drizzle.execute(sql`${sql.raw(`CREATE TABLE IF NOT EXISTS "_pages_v_version_breadcrumbs" ("_order" integer NOT NULL, "_parent_id" integer NOT NULL, "_path" text NOT NULL DEFAULT '', "_locale" "_locales" NOT NULL, "id" serial PRIMARY KEY NOT NULL, "doc_id" integer, "url" varchar, "label" varchar, "_uuid" varchar)`)}`)
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'unknown error'
    console.warn(`[migration] Skipped breadcrumbs: ${msg}`)
  }
}
